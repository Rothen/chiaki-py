...
class PSNLoginParser:
    ...
    def parse_psn_account(cls, redirect_url: str) -> PSNAccount:
        ...
        """
        code: str | None = cls.__parse_redirect_url(redirect_url)
        if code is None:
            raise ValueError("Code is None")
        token = cls.__get_token(code)
        if token is None:
            raise ValueError("Token is None")
        account = cls.__fetch_account_info(token)
    ...
    def __fetch_account_info(cls, token: str) -> PSNAccount:
        ...
        raise ValueError("Error getting account")
    ...
    def __format_psn_account_info(cls, account_info: dict[str, Any]) -> tuple[str, str]:
        ...
        user_id = account_info['user_id']
        user_b64 = cls.__format_user_id(user_id, "base64")
        user_creds = cls.__format_user_id(user_id, "sha256")
        if user_b64 is None or user_creds is None:
            raise ValueError("User ID is None")
        return user_b64, user_creds

    @classmethod
    def __format_user_id(cls, user_id: str | None, encoding: str = "base64") -> str | None:
